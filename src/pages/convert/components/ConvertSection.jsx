...
const ConvertSection = ({...props}) => {
	...
	async function getStats() {
		const activeTokenAddress = selectOptions[activeIndex].address !== "0xA1077a294dDE1B09bB078844df40758a5D0f9a27" ? selectOptions[activeIndex].address : "0x0000000000000000000000000000000000000000"
		const [_userLpBalance, _allowance, _userStockBalance, _allowanceStock, _daiPlsReserves, _daiPlsSupply, _userLocks] = await multicall({
			contracts: [
				{
		...
		const _stockBalance = new BigNumber(_userStockBalance.result ? _userStockBalance.result : 0).div(1e18)
		setStockBalance(_stockBalance);
		if(activeIndex!==0 && exchangeDirection == 'to') {
			const _lpBalance = new BigNumber(_userLpBalance.result ? _userLpBalance.result : 0).div(1e18);
			setLpBalance(_lpBalance);
			const _approved = new BigNumber(_allowance.result).greaterThan(_lpBalance.mul(1e18));
			...
			const _approved = new BigNumber(_allowanceStock.result).greaterThan(_stockBalance.mul(1e18));
			setApproved(_approved);
		}
		const _stockPrice = (Number(_daiPlsReserves.result[0]) *  Number(selectOptions[0].price) + Number(_daiPlsReserves.result[1]))/Number(_daiPlsSupply.result);
		setStockPrice(_stockPrice);
		userLocks = _userLocks.result.map((el, i) => {return {stakeId: i, ...el}}).reverse();
		userLocks = userLocks.filter((lock)=>{return lock.claimed==false})
	...
	async function convert() {
		...
		let tx;
		console.log(amountToBurn.toString())
		if(exchangeDirection==="to") {
			if(activeIndex == 0) {
				...
				tx = writeContract({
				  ...
				  account: userAccount.address,
				  args: [selectOptions[activeIndex].address, amountToZap, 20],		
				});
			}
		} else {
			if(activeIndex == 0) {
				...
				tx = writeContract({
					...
					account: userAccount.address,
					args: [amountToBurn, selectOptions[activeIndex].address, 20],		
				});
			}
		}
		getStats();
			  ...
			  description={
				<div className="font-[500] leading-[17px] text-[16px] mt-2">
				  ...
				  <span className="text-[#01BCFF]">
					{(exchangeDirection === "to" ? 
					toLocaleString(amountToZap.div(1e18), 2, 2) + selectOptions[activeIndex].title + " to STOCK" :
					toLocaleString(amountToBurn.div(1e18), 2, 2) + " STOCK to " + selectOptions[activeIndex].title)}
				  </span>
				</div>
			  }
			  hash={hash}
	...
	async function lock() {
		...
		addTransaction(userAccount.address, {
			action: "Lock",
			text:  toLocaleString(_amount.div(1e18), 2, 2) + " STOCK",
			protocol: "convert",
			transaction: hash,
			timestamp: Date.now()
		})
	...
	async function burnLocked(_stakeId) {
		...
		addTransaction(userAccount.address, {
			action: "Claim from lock",
			text:  toLocaleString(Number(userLocks.filter(e=> e.stakeId == _stakeId)[0].amount) / 1e18, 2, 2) + " STOCK",
			protocol: "convert",
			transaction: hash,
			timestamp: Date.now()
		})
				...
				<div className="container xl:max-w-[1070px] mx-auto relative z-[10]">
					<div className="flex flex-wrap gap-y-5 items-end gap-x-5 mb-8">
						...
						<div className="w-full md:w-auto text-right md:min-w-[235px] flex flex-wrap items-center gap-2 md:flex-col md:items-end justify-end">
							...
							<button className="btn-2 md:w-full md:max-w-[235px]" onClick={()=>setZappedModalOpenFromHeader(true)}>
								Buy Tokens
							</button>
						</div>
					</div>
								...
								<div className="py-6 md:py-7 h-full relative z-[11]">
									...
									<div className="px-4 xl:px-[39px] gap-5 md:gap-[27px] md:gap-y-[17px]">
													...
													<div className="rounded-[10px] bg-[#140236] text-center h-full">
															...
															<div className="pt-4 pb-2 px-[14px] w-0 flex-grow">
																<input
																	...
																	onChange={(e)=>{
																		...
																		setStockInput(val);
																		if(exchangeDirection==="to") setAmountInput(((val)*stockPrice)/selectOptions[activeIndex].price);
																		else setAmountInput(((val*0.95)*stockPrice)/selectOptions[activeIndex].price);
																	}}
																/>
																<div className="text-white text-opacity-50 text-sm font-bold text-left">
																≈ ${toLocaleString(Number(amountInput)*selectOptions[activeIndex]?.price, 2, 2)}
																</div>
															</div>
														...
														<div className="text-sm text-white text-opacity-50 border-t border-white border-opacity-20 py-1 mx-[14px] flex justify-between font-medium">
															...
															<span onClick={()=>{
																...
																setAmountInput((stockBalance*stockPrice)/selectOptions[activeIndex].price);
															}}>{toLocaleString(stockBalance, 9, 9)}</span>
														</div>
													</div>
												</div>
											</div>
												...
												<div className="asset-card shadow-none">
													<div className="rounded-[10px] bg-[#140236] text-center h-full">
															...
															<div className="pt-4 pb-2 px-[14px] w-0 flex-grow">
																<input
																	...
																	onChange={(e)=>{
																		var val = (e.target.value.replace(/\,/g, ""));
																		setAmountInput(val);
																		if(exchangeDirection === "to") setStockInput(val*selectOptions[activeIndex].price/stockPrice)
																		else setStockInput((val*1.0526316163)*selectOptions[activeIndex].price/stockPrice)
																	}}
																/>
																<div className="text-white text-opacity-50 text-sm font-bold text-left">
																	${toLocaleString(Number(amountInput)*selectOptions[activeIndex]?.price, 2, 2)}
														...
														<div className="text-sm text-white text-opacity-50 border-t border-white border-opacity-20 py-1 mx-[14px] flex justify-between font-medium">
															<span>Wallet Balance</span>
															<span onClick={()=>{
																setAmountInput(selectOptions[activeIndex].value);
																setStockInput(selectOptions[activeIndex].value*selectOptions[activeIndex].price/stockPrice)
															}}>{toLocaleString(selectOptions[activeIndex].value)}</span>
														</div>
													</div>
												</div>
											</div>
										...
										<div className="mt-6 text-white text-center text-opacity-50 text-sm font-normal">
											To avoid the 5% STOCK Sell Tax, lock your STOCK
											tokens for 14 days.
										</div>
									</div>
								</div>
							...
							<div className="pt-6 md:pt-10 pb-2 h-full relative z-[11]">
								...
								<div className="overflow-x-auto">
									...
									<table className="custom-table border-spacing-y-5">
										...
										{userLocks
											.map((lockInfo, index) => (
											<tr className="text-center" key={lockInfo.amount}>
												...
												<div className="md:font-medium text-normal text-opacity-50 text-nowrap">
													<CountdownCard
													targetDate={new Date(Number(lockInfo.timestamp)*1000).toString()}
													showDays={true}
													shortend={true}
													/>
												</div>
				...
				<div className="container relative z-10">
					...
					<div className="w-full flex flex-wrap gap-5 pt-20 md:pt-[95px] ">
						...
						<div
							...
							key={index}
						>
							<LockCard lockInfo={lockInfo} burnLocked={burnLocked} stockPrice={stockPrice}/>
						</div>
						))}
					</div>
					)}
            ...
            <div className="flex flex-wrap justify-between gap-y-4">
                    ...
                    <div className="bg-tableBg shadow-innerShadow rounded-[5px]">
                      ...
                      <p className="text-sm pb-[2px] border-t border-white border-opacity-20">
                        <span className="text-sm text-gradient-3 min:w-[155px]">
                        	${toLocaleString(Number(lockInfo.amount)/1e18*stockPrice)}
                        </span>
                      </p>
                    </div>
                  </div>
              ...
              <div className="w-full">
                <button
				  ...
				  onClick={()=> {burnLocked(lockInfo.stakeId)}}
                >
                  Claim Stake
                </button>
              </div>
